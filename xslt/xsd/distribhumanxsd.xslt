...
&lt;xsd:complexType name="<xsl:value-of select='@name'/>"&gt;
     ...
     <xsl:apply-templates select="attribute"/>             <!-- attributes after content -->
&lt;/xsd:complexType&gt;
</xsl:when>
...
<xsl:when test="@use='optional' or @use='NoSubmit'">  
          &lt;xsd:<xsl:value-of select="$var"/> minOccurs="0"/&gt;</xsl:when>
<xsl:when test="not(@use) or @use='NoSubmitDistribRequired'">  
          &lt;xsd:<xsl:value-of select="$var"/>/&gt;</xsl:when>
<xsl:when test="@use='unbounded'">  
          &lt;xsd:<xsl:value-of select="$var"/> maxOccurs="unbounded"/&gt;</xsl:when>
<xsl:when test="@use='requiredAttribute'">   <!-- for attributes only -->
          &lt;xsd:<xsl:value-of select="$var"/> use="required"/&gt;</xsl:when>
</xsl:choose> 
</xsl:template>
...
<xsl:template match="attribute[@name]|element[@name]|group[@name]"> <!-- must have type too -->
...
<xsl:template match="any"> <!-- don't bother with any options -->
    &lt;xsd:any minOccurs="0" maxOccurs="unbounded" processContents="lax"/&gt; </xsl:template>
