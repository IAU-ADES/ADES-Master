...
def processAdesElement(element, allowedObsDataSet, allowedAdesSet, allowedObsBlockSet):
    ...
    for child in element:
        ...
        if tag == "obsBlock":
            ...
            processObsBlock(
                child, allowedObsBlockSet, allowedObsDataSet
            )  # do not print data dicts inside
            lastDataTagType = tag
...
def printOpticalLine(item):
    ...
    if item["stn"] == "275":
        ...
        item["stn"] == "244"
    ):  # This has not been tested because at the moment we don't have any data
    ...
    mag = adesutility.applyPaddingAndJustification(
        hasKeyOrVal(item, "mag", ""), 5, "D", 3
    )[0]
...
def printRadarLine(item):
    ...
    od = rmsDoppler
    rmsDoppler = adesutility.applyPaddingAndJustification(rmsDoppler, 16, "D", 12)[0]
    ...
    delay = adesutility.applyPaddingAndJustification(delay, 16, "D", 6)[0]
    delay = delay[0:5] + delay[6:]  # remove '.'

    rmsDelay = hasKeyOrVal(item, "rmsDelay", "              ")
    od = rmsDelay
    rmsDelay = adesutility.applyPaddingAndJustification(rmsDelay, 15, "D", 11)[0]
    rmsDelay = rmsDelay[0:10] + rmsDelay[11:]  # remove '.'

    frq = hasKeyOrVal(item, "frq", "       ")
    od = frq
    frq = adesutility.applyPaddingAndJustification(frq, 7, "D", 6)[0]
...
def printDataDicts():
    ...
    if dataDicts:  # don't print anything if no entries ([])
        ...
        for subDict in dataDicts:
            eType = subDict[0]
            if (
                eType == "optical" or eType == "occultation"
            ):  # process optical -- may be rover or converted offset or occultation
...
def processObsContext(element):
    ...
    for i in element:
        ...
        if tag == "comment":
            ...
            tag == "observatory"
        ):  # right now the mpcCode sub-element is the one that matters
            ...
            tag == "telescope"
        ):  # right now the name sub-element is the one that matters
...
def xmltompc80col(xmlfile, mpcencdoing="utf-8"):
    ...
    processAdesElement(
        inputTree.getroot(), allowedObsDataSet, allowedAdesSet, allowedObsBlockSet
    )
