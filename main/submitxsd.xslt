...
<!-- 
     ...
     'distribOnlyRequired' is missing in submit, required in distrib, and 
     optional in general.
     'distribOnlyOptional' is missing in submit, required in distrib, and 
     optional in general.
 -->
<xsl:template name="processUse">
  <xsl:choose> <!-- add attributes for @use -->
     <xsl:when test="@use='distribOnlyRequired'">
     </xsl:when>
     <xsl:when test="@use='distribOnlyOptional'">
     </xsl:when>
...
<xsl:template match="*[self::element or self::group][@ref][not(@use='distribOnlyRequired')][not(@use='distribOnlyOptional')]">
...
<xsl:template match="*[self::element or self::group][@name][@type][not(@use='distribOnlyRequired')][not(@use='distribOnlyOptional')]"> <!-- must have both name and type -->
  <xsl:element name="xsd:{local-name()}">
     <xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute>
