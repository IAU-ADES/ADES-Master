...
<!-- 
     ...
     optional in general.
     'distribOnlyOptional' is missing in submit, optional in distrib, and 
     optional in general.
 -->
...
<xsl:when test="@use='distribOnlyOptional'"> (distribOnlyOptional) </xsl:when>
<xsl:when test="@use='distribOnlyRequired'"> (distribOnlyRequired) </xsl:when>
</xsl:choose> 
</xsl:template>

<xsl:template match="/ades/XSDTypes/*" >
\subsubsection*{<xsl:value-of select='local-name()'/>:  <xsl:value-of select='@name'/>}
