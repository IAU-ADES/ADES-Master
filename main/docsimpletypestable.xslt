...
\textbf{<xsl:value-of select='@name'/>} <xsl:apply-templates select="restriction"/> &amp; <xsl:value-of select='doc'/> \\ 
\hline
</xsl:for-each>

<xsl:for-each select="/ades/XSDTypes/simpletype[url]" >
\textbf{<xsl:value-of select='@name'/>} <xsl:apply-templates select="restriction"/> &amp; <xsl:value-of select='doc'/> at <xsl:value-of select='url'/> \\ 
\hline
</xsl:for-each>
